...
def dijkstra(G, s):
  ...
  dist[s] = 0
  q = PriorityQueue()
  q.put((0, s, 0))          #dystans wierzcholek ile nie spi

  while not q.empty():
    crr_distance, v, awake = q.get()

    if crr_distance > dist[v]:
      continue

    for u, distance in G[v]:
      if awake + distance > 16:
        if dist[u] > dist[v] + distance + 8:
          dist[u] = dist[v] + distance + 8
          q.put((dist[u], u, distance))
      else:
        if dist[u] > dist[v] + distance:
          dist[u] = dist[v] + distance
          q.put((dist[u], u, awake + distance))
